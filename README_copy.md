...
$$
\mu_t^{(i)} =
\mu_t^{(i)} + p(z_{t} \mid m_{t-1}^{(i)}, x_j) p(x_t \mid x_{t-1}^{(i)}, u_t)
$$

$$
\eta^{(i)} =
\eta^{(i)} + p(z_{t} \mid m_{t-1}^{(i)}, x_j) p(x_t \mid x_{t-1}^{(i)}, u_t)
$$

Normalize the mean:

$$
\mu_t^{(i)} = \frac{\mu_t^{(i)}}{\eta^{(i)}}
$$
...
=
\Sigma_t^{(i)} +
(x_j - \mu_t) \cdot (x_j - \mu_t)^T \cdot
p(z_{t} \mid m_{t-1}^{(i)}, x_j) \cdot
p(x_t \mid x_{t-1}^{(i)}, u_t)
$$
