...
class OGMROSCommunication():
    def __init__(self, map_parameter, occupancy_parameter, ros_parameter, sensor_parameter):
        self.ogm= OccupancyGridMapping(map_parameter, occupancy_parameter, ros_parameter, sensor_parameter)
        self.ogm.create_map()
    ...
    def publish_occupancy_grid_message(self):
        '''Get's the logOdds map and do all necessary transformation's for publishing the Occupancy Message.'''
        # Copy the logOdds map to the message
        self.ogm.log_odds_map_msg.data= self.ogm.log_odds_map.ravel() 
        # generate timestamp
        self.ogm.log_odds_map_msg.header.stamp= rospy.Time.now()
        self.map_publisher.publish(self.ogm.log_odds_map_msg)
