...
'''
Description (without map extension)

Program calculates the grid map in logOdds form. The logOdds map will be published after updating the map.
...
class OccupancyGridMapping():
    IDX_X= 0
    IDX_Y= 1
    def __init__(self, map_width_m= 10.0, map_height_m= 10.0, grid_resolution_m= 1.0, map_prior_probability= 0.5, 
        ...
        # Define may and min value for logOdds 
    ...
    def create_map(self):
        ...
        number_of_grids_x= ceil(self.map_width_m / self.grid_resolution_m)        
        # Check for odds number of grid cells
        if(not (number_of_grids_x % 2)):
            number_of_grids_x+= 1
        # Update map width
        self.map_width_m= number_of_grids_x * self.grid_resolution_m
        # Define number of grids in y direction(must be odd value)
        number_of_grids_y= ceil(self.map_height_m / self.grid_resolution_m)        
        # Check for odds number of grid cells
        if(not (number_of_grids_y % 2)):
            number_of_grids_y+= 1
        # Update map Height
        self.map_height_m= number_of_grids_y * self.grid_resolution_m
        # Create map and initialize prior probability 
        self.log_odds_map= np.full((number_of_grids_y, number_of_grids_x), self.log_odds_prior)
        ...
        self.init_occupancy_grid_message(number_of_grids_x, number_of_grids_y)
        # rospy.loginfo("\n\nNumber of grid cells in width and height: %i %i \n\n", number_of_grids_x, number_of_grids_y)

    
    def init_occupancy_grid_message(self, number_of_grids_x, number_of_grids_y):
        '''Init the static values of the OccupancyGrid message.'''
        self.log_odds_map_msg.info.width= number_of_grids_x
        self.log_odds_map_msg.info.height= number_of_grids_y
    ...
    def execute(self):
        ...
        while not rospy.is_shutdown():
            ...
            if(self.geometry_pose and self.laser_scan):
                ...
                self.pose= self.transform_pose_to_planar_pose(self.geometry_pose)
                self.measurements= self.transform_laser_scan_to_measurement(self.laser_scan)
                self.lock.release()
                # Test reflecting grid cell methode
                self.update_map(self.measurements, self.pose)
            ...
            # self.print_planar_pose(self.pose)
...
def main():
     # Init Node
    rospy.init_node("occupancy_grid_mapping", anonymous=True)
    # Define map (size in mm)
    map_width_m= 15.0               # [m] -> 20 m
    map_height_m= 25.0              # [m] -> 20 m
    ...
    grid_map= OccupancyGridMapping(map_width_m= map_width_m, map_height_m= map_height_m, grid_resolution_m= grid_resolution_m,
                                    map_prior_probability= prior_probability, increasing_probability= 0.65, 
                                    decreasing_probability= 0.35, odom_topic= odom_topic, scan_topic= scan_topic,
                                    update_rate= update_rate, min_sensor_range= min_sensor_range, max_sensor_range= max_sensor_range)
    grid_map.create_map()
    # Start the algorithm
    grid_map.execute()
