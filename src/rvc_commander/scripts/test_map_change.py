    ...
    def execute(self):
        ...
        while not rospy.is_shutdown():
            ...
            if(self.geometry_pose and self.laser_scan):
                ...
                pose= self.transform_pose_to_planar_pose(self.geometry_pose)
...
def test_map_extension_whole_map():
     # Init Node
    rospy.init_node("test_map_change", anonymous=True)
    # Define map (size in mm)
    map_width_m= 10.0               # [m] -> 20 m
    map_height_m= 10.0              # [m] -> 20 m
    grid_resolution_m= 1.0         # [m] -> 50 mm grid resolution
    min_distance_to_border= 5.0    # The minimum distance from the actual robot pose to the border before extending the map
    prior_probability= 0.5          # Init map with probability of 0.5
    # Define subscriber topics
    odom_topic= "true_odom"
    scan_topic= "scan"
    ...
    grid_map= OccupancyGridMapping(map_width_m= map_width_m, map_height_m= map_height_m, grid_resolution_m= grid_resolution_m,
                                    ...
                                    max_sensor_range= max_sensor_range)
    grid_map.create_map()
    # Colorize all cells black
    full_occupied_map(grid_map)
    # Publish map 
    while(not (grid_map.geometry_pose and grid_map.laser_scan)):
        pass
    grid_map.publish_occupancy_grid_message()    
    # Extend the map 
    pose= (0.0, 0.0, 0.0)
    extension_needed= True
    while(extension_needed):
        extension_needed= grid_map.map_extansion_if_necessary(pose)
    # Publish map 
    grid_map.publish_occupancy_grid_message()    
    # while loop
    # Extend map the second time
    rospy.loginfo("sleep")    
    time.sleep(8) 
    rospy.loginfo("awake")
    pose= (5.0, 0.0, 0.0)
...
def test_map_extension_cell_wise():
    # Init Node
    rospy.init_node("test_map_change", anonymous=True)
    # Define map (size in mm)
    map_width_m= 10.0               # [m] -> 20 m
    map_height_m= 10.0              # [m] -> 20 m
    grid_resolution_m= 1.0         # [m] -> 50 mm grid resolution
    min_distance_to_border= 4.5    # The minimum distance from the actual robot pose to the border before extending the map
    prior_probability= 0.5          # Init map with probability of 0.5
    # Define subscriber topics
    odom_topic= "true_odom"
    scan_topic= "scan"
    ...
    # pose= [0.0, 5.0, 0.0]
    pose= [0.0, -5.0, 0.0]
    pos_cell= grid_map.transform_point_to_grid_cell(pose[:2])
    # Colorize robot pos cell black 
    grid_map.colorize_grid_black(pos_cell)
    # Publish the map
    while(not (grid_map.geometry_pose and grid_map.laser_scan)):
        pass
    grid_map.publish_occupancy_grid_message()   
    ...
    while(extension_needed):
        extension_needed= grid_map.map_extansion_if_necessary(pose)
    # Colorize the array again
    new_pos_cell= grid_map.transform_point_to_grid_cell(pose[:2])
    grid_map.colorize_grid_black(new_pos_cell)
    # Publish grid map
    grid_map.publish_occupancy_grid_message()   
