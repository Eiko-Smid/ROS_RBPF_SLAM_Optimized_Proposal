...
Description:
    ...
    Monte Carlo Localization Algorithm with uses a pre computed likelihood field as the measurement model. Unfortunately the pre computation
...
class ParticleFilter():
    def __init__(self, particle_filter_parameter, start_pose, map_parameter_particle_filter, occupancy_values, occupancy_grid_map): 
        ...
        self.control_motion_factor, self.control_turn_factor, self.measurement_distance_stddev= uncertainty_parameter
        # Map parameter
        ...
        self.min_sensor_range, self.max_sensor_range, self.z_hit, self.z_random= measurement_parameter
        # Likelihood field
        # Find all occupied cells in map and save their (x, y) coordinate as well as the cell indice in the map.
        self.occupied_cells= self.find_occupied_cells()
        self.occupied_cell_points= [point for point, indices in self.occupied_cells]
        # Initialize nearest neighbor object
        self.neighbor= NearestNeighbors(n_neighbors=1, algorithm='ball_tree')
        self.neighbor.fit(self.occupied_cell_points)
        # Resampling 
    ...
    @staticmethod
    def calculate_measurement_reflection_point(pose, measurement):
        '''Get's a (x, y, theta) pose and a (range, bearing) measurement and calculates the reflection 
        point of the beam. Returns a tuple consisting of the xy-coordinate of the reflecting point.'''
        x, y, heading= pose
        ...
        # rospy.loginfo("reflection x= %f", reflection_point_x)
        # rospy.loginfo("reflection y= %f", reflection_point_y)
        return (reflection_point_x, reflection_point_y)
    ...
    def compute_weights_test(self, measurements):
        ...
        z_random_by_max_range= self.z_random / 0.01 
        # Compute likelihood field
        if(valid_measurements):
            for i in range(len(self.particles)):
                counter= 0
                average_endpoint_to_occ_cell_dist= 0
                weight= 1.0
                ...
                if(self.is_cell_inside_map(particle_cell_index)):
                    for m in valid_measurements:
                        cell_unknown= False
                        cell_inside_map= False
                        # Calculate measurement endpoint 
                        reflection_point= self.calculate_measurement_reflection_point((x, y, theta), m)
                        # Check if point is inside map 
                        row, column= self.transform_point_to_grid_cell(reflection_point)
                        if(self.is_cell_inside_map((row, column))):   
                            cell_inside_map= True 
                            occupancy_value= self.occupancy_grid_map[row][column]
                            # if cell is occupied 
                            if(occupancy_value == self.occ):
                                distance= 0.00                                
                            # if cell is free
                            elif(occupancy_value == self.free):
                                distance, index= self.neighbor.kneighbors([reflection_point])                                      
                            else:
                                '''For testing'''
                                distance, index= self.neighbor.kneighbors([reflection_point])                                  
                                cell_unknown= True
                        # Calculate likelihood field probability
                        if(cell_inside_map and not cell_unknown):
                            average_endpoint_to_occ_cell_dist+= distance    
                            p_hit= normal_dist.pdf(distance, 0, self.measurement_distance_stddev)
                            weight*= self.z_hit * p_hit + z_random_by_max_range
                            counter+= 1
                        # For uncertain areas in the map
                        elif(cell_inside_map and cell_unknown):
                            '''Normally it should be 1/z_max.'''
                            # weight*= 1/self.max_sensor_range
                            '''For testing '''
                            average_endpoint_to_occ_cell_dist+= distance    
                            p_hit= normal_dist.pdf(distance, 0, self.measurement_distance_stddev)
                            weight*= self.z_hit * p_hit + z_random_by_max_range
                            counter+= 1
                            # rospy.loginfo("cell unknown")
                        else:
                            # What to do when cell is not inside of the map ?
                            # Seems to be that this is not that often the case
                            # rospy.loginfo("status: %i", cell_inside_map)
                            pass
                    # Update current particle weight with new particle weight
                    # rospy.loginfo("New p_weight= %f", weight)
                    # rospy.loginfo("Average distance= %f", average_endpoint_to_occ_cell_dist/counter)
                    # rospy.loginfo("counter= %i", counter)
                    self.weights[i]= self.weights[i] * weight
                else: 
                    rospy.loginfo("Particle out of map.")
                    self.weights[i]= 0.0
            # Normalize weights
            sum_of_weights= sum(self.weights)
            if(sum_of_weights > 0.0):
                self.weights= [weight/sum_of_weights for weight in self.weights]
            else:
                rospy.loginfo("\nweights are zero\n")

    
    def compute_weights_clean(self, measurements):
        '''Pre computes the likelihood field of the map.'''
        # Exclude invalid measurements
        valid_measurements= self.exclude_invalid_measurements(measurements)
        z_random_by_max_range= self.z_random / self.max_sensor_range
        # Compute likelihood field
        if(valid_measurements):
            for i in range(len(self.particles)):                
                weight= 1.0
...
def main():
    ...
    measurement_distance_stddev = 0.3               # Distance measurement error [m].
    # Measurement parameter
    min_sensor_range= 0.1                           # Min measurement range [m]
    max_sensor_range= 8.0                           # Max measurement range [m]
    z_hit= 0.95                                     # z_hit parameter for measurement model
    z_random= 0.05                                  # z_rand parameter for measurement model
    measurement_parameter= (min_sensor_range, max_sensor_range, z_hit, z_random)
    # Summarize particle filter parameter
    uncertainty_parameter= (control_motion_factor, control_turn_factor, measurement_distance_stddev)
