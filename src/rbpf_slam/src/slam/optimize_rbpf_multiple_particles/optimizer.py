...
class RBPFOptimizer:
    ...
    def generate_params_for_hash(exp_params: ExperimentParams) -> dict:
        ...
        parameter_for_hash = {
            ...
            "cov_std_scale": exp_params.cov_std_scale,
            "cov_max_std_xy": exp_params.cov_max_std_xy,
            "cov_max_std_theta": exp_params.cov_max_std_theta,
            "min_std_xy": exp_params.min_std_xy,
    ...
    def optimize(
        ...
        for params in tqdm(params_list, total=total_runs, desc="RBPF optimization", unit="run"):
            ...
            for run_seed in seed_list:
                ...
                run_result = self.runner.run_rbpf_parallel(
                    run_playback_data,
                    params,
                    run_seed=run_seed,
                )
                ...
                # Store the particle history and final highest-weighted particle map.
                if store_map_data:
                    seed_part = str(run_seed) if run_seed is not None else "none"
                    ds_id_part = str(dataset_id) if dataset_id is not None else "unknown_dataset"
                    map_part = str(map_name) if map_name is not None else "unknown_map"
