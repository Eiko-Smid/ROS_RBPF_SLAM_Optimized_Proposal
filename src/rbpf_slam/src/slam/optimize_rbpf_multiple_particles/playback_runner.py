...
class PlaybackRunner:
    ...
    def run(self, playback_data: PlaybackData, params: ExperimentParams) -> RunResult:
        ...
        best_p_map = info.get("best_particle_map", None)
        best_p_map_metadata = info.get("best_particle_map_meta", None)
        run_result.best_part_map = best_p_map
        run_result.best_part_map_meta = best_p_map_metadata
    ...
    def run_rbpf_threading(self, playback_data: PlaybackData, params: ExperimentParams) -> RunResult:
        """
        Executes one full RBPF run over all playback steps and returns evaluated results. This version uses
        the RBPF parallel step method with a threading pool for parallel particle updates. 
        """
        # Create rbpf instance for the current parameter set
        rbpf = self.factory.create(
            scan_match_fac=ScanMatchFactory(),
